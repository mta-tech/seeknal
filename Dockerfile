...
lines = []; \
[lines.append(d.split(';')[0].strip()) for d in deps \
 if re.split(r'[><=!~\[]', d)[0].strip().lower() not in skip \
 and 'extra ==' not in d]; \
