...
def _scaffold_agent_guidance(
    ...
    *,
    ...
    """Create project-local guidance files for coding/agent assistants."""
    files = {
        "AGENTS.md": _project_agents_md(project_name),
...
def _project_claude_md(project_name: str) -> str:
...
"""
        ...
        typer.echo("")
        _echo_info("Ask / read-only database workflow:")
        typer.echo(
            "  - Connect source:      "
            "seeknal source connect <name> --connector postgresql --dsn-env WAREHOUSE_URL"
        )
        typer.echo("  - Sync context:        seeknal source sync <name> --project .")
        typer.echo("  - Chat with data:      seeknal ask chat --project .")
        typer.echo("  - Run Ask tests:       seeknal ask test --project . --sql-only")
        typer.echo("")
        _echo_info("Advanced:")
